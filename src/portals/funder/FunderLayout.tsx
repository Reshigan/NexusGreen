...
const FunderLayout: React.FC<FunderLayoutProps> = ({ children }) => {
  const [sidebarOpen, setSidebarOpen] = useState(false);
  const { user, logout, switchPortal, getAccessiblePortals } = useMultiPortalAuth();
  const location = useLocation();

  const navigation = [
    { name: 'Dashboard', href: '/funder/dashboard', icon: Home, current: location.pathname === '/funder/dashboard' },
    { name: 'Portfolio', href: '/funder/portfolio', icon: PieChart, current: location.pathname === '/funder/portfolio' },
    { name: 'ROI Analysis', href: '/funder/roi', icon: TrendingUp, current: location.pathname === '/funder/roi' },
    { name: 'Analytics', href: '/funder/analytics', icon: BarChart3, current: location.pathname === '/funder/analytics' },
  ];
      ...
      <div className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-64 lg:flex-col">
        <div className="flex flex-col flex-grow bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700">
          <div className="flex h-16 items-center px-4 border-b border-slate-200 dark:border-slate-700">
            <img src="/nexus-green-logo.svg" alt="NexusGreen" className="h-8 w-auto" />
            <Badge variant="secondary" className="ml-2 text-xs">Funder</Badge>
          </div>
          <nav className="flex-1 px-4 py-4 space-y-1">
            {navigation.map((item) => {
              const Icon = item.icon;
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-700">
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56">
                  {accessiblePortals.filter(portal => portal !== UserRole.FUNDER).map((portal) => (
            ...
            <div className="flex items-center">
              ...
              <div className="hidden sm:block ml-4">
                ...
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  Monitor your solar energy investments
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="hidden md:flex items-center space-x-6 text-sm">
                <div className="text-center">
                  <p className="font-medium text-slate-900 dark:text-slate-100">8</p>
                  <p className="text-slate-500 dark:text-slate-400">Projects</p>
                </div>
                <div className="text-center">
                  <p className="font-medium text-slate-900 dark:text-slate-100">R 45.2M</p>
