...
exports.SDGTrackingService = void 0;
const client_1 = require("@prisma/client");
const logger_1 = require("../utils/logger");
const errorHandler_1 = require("../utils/errorHandler");
const solarDataService_1 = require("./solarDataService");
const prisma = new client_1.PrismaClient();
const solarDataService = new solarDataService_1.SolarDataService();
class SDGTrackingService {
    /**
     * Calculate SDG impact metrics for an organization
     */
    async calculateSDGImpact(organizationId) {
        try {
            ...
            for (const site of sites) {
                if (site.solaxClientId && site.solaxClientSecret && site.solaxPlantId) {
                    try {
                        const energyData = await solarDataService.getEnergyData(site.id, startDate, endDate);
                        ...
                        logger_1.logger.error(`Failed to get energy data for SDG calculation for site ${site.id}`, {
            ...
            const metrics = await this.calculateSDGMetrics(totalCapacity, totalGeneration, totalCO2Avoided, sites.length);
            return {
                organizationId,
            ...
            logger_1.logger.error('Failed to calculate SDG impact', {
                organizationId,
                error: (0, errorHandler_1.getErrorMessage)(error)
            });
            throw error;
     ...
     */
    async calculateSDGMetrics(totalCapacity, totalGeneration, totalCO2Avoided, siteCount) {
        const metrics = [];
     ...
     */
    async getSDGComparison(organizationIds) {
        try {
            const impacts = await Promise.all(organizationIds.map(id => this.calculateSDGImpact(id)));
            const aggregated = {
            ...
            logger_1.logger.error('Failed to get SDG comparison', { error: (0, errorHandler_1.getErrorMessage)(error) });
            throw error;
        }
    }
    /**
     ...
     */
    async generateSDGReport(organizationId, startDate, endDate) {
        try {
            ...
            for (const site of organization.sites) {
                if (site.solaxClientId && site.solaxClientSecret && site.solaxPlantId) {
                    try {
                        const energyData = await solarDataService.getEnergyData(site.id, previousStart, previousEnd);
                        const generation = energyData.totalGeneration || 0;
                        previousGeneration += generation;
                        previousCO2Avoided += generation * 0.5;
                    }
                    catch (error) {
                        logger_1.logger.error(`Failed to get previous period data for site ${site.id}`, {
                            error: (0, errorHandler_1.getErrorMessage)(error)
                        });
                    }
                }
            ...
            logger_1.logger.error('Failed to generate SDG report', {
                organizationId,
                error: (0, errorHandler_1.getErrorMessage)(error)
            });
            throw error;
     ...
     */
    generateRecommendations(impact, trends) {
        const recommendations = [];
        // Energy generation recommendations
        if (trends.energyGeneration.change < 5) {
            recommendations.push('Consider expanding solar capacity or optimizing existing systems to increase renewable energy generation (SDG 7)');
        }
        // Climate action recommendations
        if (impact.summary.totalCO2Avoided < impact.totalCapacity * 1000) {
            recommendations.push('Maximize CO2 impact by ensuring optimal system performance and consider additional climate mitigation measures (SDG 13)');
        }
        // Economic growth recommendations
        if (impact.summary.jobsSupported < impact.totalCapacity * 0.15) {
            recommendations.push('Explore opportunities to create more local jobs through maintenance, training, and community programs (SDG 8)');
        }
        // Community impact recommendations
        if (impact.summary.communitiesImpacted < impact.totalSites) {
            recommendations.push('Develop community engagement programs to maximize local benefits and sustainable development impact (SDG 11)');
        }
        // Innovation recommendations
        recommendations.push('Consider implementing smart grid technologies and energy storage to enhance infrastructure resilience (SDG 9)');
        return recommendations;
    }
    /**
     * Get SDG alignment score for an organization
     */
    async getSDGAlignmentScore(organizationId) {
        try {
            ...
            logger_1.logger.error('Failed to calculate SDG alignment score', {
                organizationId,
                error: (0, errorHandler_1.getErrorMessage)(error)
            });
            throw error;
...
exports.SDGTrackingService = SDGTrackingService;
//# sourceMappingURL=sdgTracking.js.map