...
class TariffEngine {
     ...
     */
    getTariffPeriod(time, timePeriods = this.defaultTimePeriods) {
     ...
     */
    async getMunicipalRates(address, municipality = null) {
        try {
            ...
            if (municipality) {
                ...
                Object.keys(baseRates).forEach(key => {
                    baseRates[key] *= adjustments[key] || 1.0;
                });
            }
            logger_1.logger.info('Retrieved municipal rates', { address, municipality, rates: baseRates });
            return baseRates;
        }
        catch (error) {
            logger_1.logger.error('Failed to get municipal rates', { error: (0, errorUtils_1.getErrorMessage)(error), address });
     ...
     */
    getMunicipalityAdjustments(municipality) {
        const adjustments = {
            'city-of-cape-town': {
                peak: 1.05,
                standard: 1.02,
        ...
        return adjustments[municipality.toLowerCase()] || {
            peak: 1.0,
            standard: 1.0,
            offPeak: 1.0,
            feedIn: 1.0
     ...
     */
    calculateSavings(energyData, rates, timePeriods = this.defaultTimePeriods) {
        try {
            let totalGridCost = 0;
            let totalSolarSavings = 0;
            let totalFeedInEarnings = 0;
            const season = this.getCurrentSeason();
            const seasonalAdj = this.seasonalAdjustments[season];
            ...
            if (energyData.hourlyData && Array.isArray(energyData.hourlyData)) {
                energyData.hourlyData.forEach(hourData => {
                    ...
                    const rate = rates[period] * (seasonalAdj[period] || 1.0);
            ...
            logger_1.logger.error('Failed to calculate savings', { error: (0, errorUtils_1.getErrorMessage)(error) });
            throw error;
        }
    }
    /**
     ...
     */
    async calculatePeriodSavings(siteId, energyData, address, period = 'day') {
        try {
            ...
            logger_1.logger.info('Calculated period savings', { siteId, period, savings: periodSavings });
            return periodSavings;
        }
        catch (error) {
            logger_1.logger.error('Failed to calculate period savings', { error: (0, errorUtils_1.getErrorMessage)(error), siteId, period });
            throw error;
        }
    }
    /**
     ...
     */
    getUsageRecommendations(energyData, rates, timePeriods = this.defaultTimePeriods) {
        try {
            ...
            logger_1.logger.error('Failed to get usage recommendations', { error: (0, errorUtils_1.getErrorMessage)(error) });
            return [];
        }
    }
}
exports.default = new TariffEngine();
//# sourceMappingURL=tariffEngine.js.map