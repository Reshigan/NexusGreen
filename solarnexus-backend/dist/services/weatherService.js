...
class WeatherService {
    constructor() {
        ...
        if (!this.apiKey) {
            logger_1.logger.warn('OpenWeatherMap API key not configured');
        }
    }
    /**
    ...
    async getWeatherForecast(latitude, longitude, days = 7) {
        try {
            const response = await axios_1.default.get(`${this.oneCallUrl}`, {
                params: {
                    lat: latitude,
                    lon: longitude,
                    appid: this.apiKey,
    ...
    async getHistoricalWeather(latitude, longitude, timestamp) {
        try {
            ...
            const response = await axios_1.default.get(`${this.oneCallUrl}/timemachine`, {
                params: {
                    lat: latitude,
                    lon: longitude,
                    dt: unixTimestamp,
            ...
            logger_1.logger.error('Error fetching historical weather:', error);
     ...
     */
    calculateSolarIrradiance(weather, latitude, longitude, timestamp = new Date()) {
        // Solar constant (W/m²)
        ...
        const elevationAngle = Math.asin(Math.sin(declination * Math.PI / 180) * Math.sin(latitude * Math.PI / 180) +
            Math.cos(declination * Math.PI / 180) * Math.cos(latitude * Math.PI / 180) * Math.cos(hourAngle * Math.PI / 180)) * 180 / Math.PI;
        // Calculate air mass
     ...
     */
    async getSolarWeatherImpact(latitude, longitude, timestamp = new Date()) {
        try {
            ...
            logger_1.logger.error('Error calculating solar weather impact:', error);
            throw error;
        }
    }
    /**
     ...
     */
    async getOptimalSolarForecast(latitude, longitude, days = 7) {
        try {
            ...
            logger_1.logger.error('Error getting optimal solar forecast:', error);
            throw error;
        }
    }
}
