...
class PredictiveAnalyticsService {
     ...
     */
    async analyzeSitePerformance(siteId) {
        try {
            ...
            logger_1.logger.error('Failed to analyze site performance', {
                siteId,
     ...
     */
    async analyzePerformanceDegradation(site, energyData) {
        try {
            ...
            logger_1.logger.error('Failed to analyze performance degradation', { error: (0, errorHandler_1.getErrorMessage)(error) });
            return null;
        }
    }
    /**
     ...
     */
    async analyzeEquipmentHealth(site, energyData) {
        try {
            ...
            logger_1.logger.error('Failed to analyze equipment health', { error: (0, errorHandler_1.getErrorMessage)(error) });
            return null;
        }
    }
    /**
     ...
     */
    async analyzeMaintenanceNeeds(site, energyData) {
        try {
            ...
            logger_1.logger.error('Failed to analyze maintenance needs', { error: (0, errorHandler_1.getErrorMessage)(error) });
            return null;
        }
    }
    /**
     ...
     */
    async analyzeWeatherImpact(site, energyData) {
        try {
            ...
            logger_1.logger.error('Failed to analyze weather impact', { error: (0, errorHandler_1.getErrorMessage)(error) });
            return null;
        }
    }
    /**
     ...
     */
    async getSystemHealthOverview(organizationId) {
        try {
            ...
            const healthOverviews = await Promise.all(sites.map(async (site) => {
                try {
                    ...
                    logger_1.logger.error(`Failed to get health for site ${site.id}`, { error: (0, errorHandler_1.getErrorMessage)(error) });
                    return {
                        siteId: site.id,
            ...
            logger_1.logger.error('Failed to get system health overview', { error: (0, errorHandler_1.getErrorMessage)(error) });
            throw error;
        }
    }
    /**
     ...
     */
    async sendAlertNotifications(predictions, siteId) {
        try {
            ...
            logger_1.logger.info('Alert notifications sent', {
                siteId,
            ...
            logger_1.logger.error('Failed to send alert notifications', {
                siteId,
                error: (0, errorHandler_1.getErrorMessage)(error)
            });
        }
    ...
    // Helper methods
    calculateDailyAverages(hourlyData) {
        const dailyGroups = {};
        ...
        return Object.values(dailyGroups).map(day => day.reduce((sum, val) => sum + val, 0) / day.length);
    }
    ...
    detectAnomalies(data) {
        if (data.length < 24)
            return { count: 0, averageImpact: 0 };
    ...
    getSeasonalExpectedPerformance(season) {
    ...
    generateAlertEmail(predictions) {
        let html = `
